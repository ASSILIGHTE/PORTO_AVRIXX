...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-24 relative bg-[#08090f] border-t border-purple-500/20 bg-pro-grid">
          ...
          <AnimatePresence mode="popLayout">
            {getFilteredProjects().map((project, idx) => (
              <motion.div
                ...
                className="group rounded-2xl bg-[#0f111e] border border-purple-500/30 overflow-hidden flex flex-col justify-between pro-card hud-corner cursor-pointer"
              >
                <div>
                  {/* Banner Image */}
                  <div className="relative aspect-video w-full overflow-hidden bg-[#08090f] border-b border-purple-500/20">
                    <img
                      src={project.image}
                      alt={project.title}
                    ...
                    <div className="absolute top-4 left-4">
                      <motion.span 
                        whileHover={{ scale: 1.05 }}
                        className="px-3 py-1 rounded-full bg-[#08090f]/90 border border-purple-500/30 text-[11px] font-mono text-purple-300 font-bold shadow-sm inline-block"
                      >
                        {project.category}
                      </motion.span>
                    </div>
                    ...
                    {project.featured && (
                      <div className="absolute top-4 right-4">
                        <span className="px-2.5 py-1 rounded-full bg-purple-600/30 border border-purple-400/40 text-[10px] font-mono text-purple-200 font-bold flex items-center gap-1">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {project.technologies.map((tech, tIdx) => (
                        <motion.span
                          ...
                          className="px-2.5 py-1 rounded-lg bg-[#08090f] border border-purple-500/20 text-[11px] font-mono font-medium text-slate-200 cursor-pointer"
                        >
                          {tech}
                        </motion.span>
                      ))}
                ...
                <div className="p-6 pt-0 mt-2 flex flex-col sm:flex-row items-center gap-3">
                  <motion.a
                    ...
                    className="w-full sm:flex-1 py-2.5 rounded-xl bg-[#08090f] hover:bg-[#151829] border border-purple-500/30 text-slate-200 hover:text-white text-xs font-bold flex items-center justify-center gap-2 transition-colors"
                  >
                  ...
                  {project.pdkiUrl ? (
                    <motion.a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full sm:w-auto py-2.5 px-4 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-black flex items-center justify-center gap-1.5 transition-colors shrink-0 shadow-md shadow-purple-950/40"
                    >
                    ...
                    <div className="w-full sm:w-auto py-2.5 px-4 rounded-xl bg-[#08090f] border border-purple-500/20 text-slate-500 text-xs font-mono font-medium flex items-center justify-center gap-1.5 shrink-0">
