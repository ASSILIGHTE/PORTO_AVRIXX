...
export default function TickerBanner() {
  const tickerItems = [
    { text: 'PLAYER 1: AUFARIQ RAJABI FRISTYAN', icon: <Gamepad2 className="w-4 h-4 text-sky-400" /> },
    ...
    { text: 'FULL STACK & BACKEND ARCHITECT', icon: <Code2 className="w-4 h-4 text-sky-400" /> },
  ...
  return (
    <div className="w-full bg-[#05060a] border-y border-sky-500/30 py-3 overflow-hidden select-none relative z-20">
        ...
        >
          {marqueeList.map((item, idx) => (
            <div key={idx} className="flex items-center gap-3 font-mono text-xs font-bold text-slate-200">
              <span className="p-1 rounded bg-sky-500/10 border border-sky-500/30">
                {item.icon}
              </span>
              <span className="tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-r from-sky-300 via-white to-emerald-300">
                {item.text}
              </span>
              <span className="text-sky-500/40 text-sm">◆</span>
            </div>
          ))}
        </motion.div>
      </div>
